...
import { CuentaActivaResponse, CuentaApiService } from '../../../services/cuenta-api.service';
...
export class MenuCamarero implements OnInit, OnDestroy {
  ...
  confirmarPedido(): void {
    ...
    this.pedidosApiService
      ...
      .subscribe({
        next: () => {
          this.platos.update((lista) => lista.map((plato) => ({ ...plato, cantidad: 0 })));
  ...
  private cargarMenu(mostrarLoading: boolean): void {
    ...
    const cantidadesPrevias = new Map(this.platos().map((plato) => [plato.id, plato.cantidad]));
  ...
  private ordenarPorCategoriaYNombre(a: PlatoApi, b: PlatoApi): number {
    ...
    const diferenciaCategoria = ordenCategorias[a.categoria] - ordenCategorias[b.categoria];
