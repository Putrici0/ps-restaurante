...
export class PlatosComponent implements OnInit, OnDestroy {
  ...
  iniciarPolling() {
    ...
    this.pollingSub = timer(0, 5000).pipe(
      ...
      switchMap(() => this.ordenesApi.obtenerPlatosSala().pipe(
        catchError(() => {
  ...
  entregarPlato(orden: OrdenCocinaResponse) {
    ...
    this.ordenesApi.marcarEntregada(orden.id).pipe(take(1)).subscribe({
      next: (ordenActualizada) => {
        this.ordenes.update(l => l.map(o => o.id === ordenActualizada.id ? ordenActualizada : o));
  ...
  deshacerEntrega(orden: OrdenCocinaResponse) {
    ...
    this.ordenesApi.deshacerEntregaPlato(orden.id).pipe(take(1)).subscribe({
      next: (ordenActualizada) => {
        this.ordenes.update(l => l.map(o => o.id === ordenActualizada.id ? ordenActualizada : o));
        this.procesandoOrdenId.set(null);
      },
